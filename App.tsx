                ...
                {isExpanded && (
                  <div className="grid gap-8">
                    ...
                    <motion.div 
                      ...
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="glass p-4 sm:p-6 rounded-[2.5rem] relative overflow-hidden group min-h-[160px] flex items-center justify-center"
                    >
                      <button 
                        onClick={() => manageNotes(item.id!)}
                        className={`absolute top-6 right-6 p-2 rounded-xl transition-all z-10 ${theme === 'dark' ? 'text-accent hover:bg-white/5' : 'text-zinc-900 hover:bg-zinc-100'}`}
                        title="Gestisci Note"
                      >
